...
int update_cases_eau(char tab[15][19])
{
    ...
    for(i=0; i<15; i++)
    {
        for(j=0; j<19; j++)
        {
            if((tab[i][j]==-1)||(tab[i][j]==2)||(tab[i][j]==66)) val++;
        }
    }
    return val;
}
...
//si l'utilisateur appuie sur une case
    if (kbhit())
    {
        ...
        *emplacement= *emplacement +1;
        ...
        if ((changement[*emplacement][0]>=0)&&(changement[*emplacement][0]<19)&&(changement[*emplacement][1]>=0)&&(changement[*emplacement][1]<15)&&(test))
        {
